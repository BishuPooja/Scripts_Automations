...
const TOKEN = "Beaer eyJhbGciOiJIUzI1NiJ9..zLan3Z24Dz7CrCgjQnXboR68QAnWKbg9vqANkcRDe0E"
let orders = []
let $event = {
    body: {
        shipment: "",
        shipmentId: "0031a865-7a5a-43bb-bdaa-d67cb54f525b"
    }
}
async function getShByShId(shId) {
    try {
        ...
        if (res?.status == 200) {
            ...
            console.log(`error in getting Shipment ${shId} ${e.message}`)
        }
    } catch (e) {
        console.log(`Error in Getting sh ${e.message}`)
    }
    return null
}
...
async function test() {
    try {
        ...
        let shipment = $event?.body.shipment
        if (shipment) {
            await main(shipment)
        } else if (shId) {
            let shRes = await getShByShId(shId)
            if (shRes) {
                console.log(`Executing Custom Action`)
                await main(shRes)
            }
        } else {
            console.log(`shipment and ShId not found`)
        }
    } catch (e) {

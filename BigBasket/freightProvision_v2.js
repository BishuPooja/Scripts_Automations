...
const token =
  "Beaer eyJhbGciOiJIUzI1NiJ9..QhhipbIJe0x9xhsuZeGmcwq2A3ThrMQmwUQebvrRVSk";
...
async function getCnsMastersFromUUIDs(cnIdsWithShs) {
  ...
  for (let i = 0; i < numBatches; i++) {
    ...
    res = res.data.map((cn) => {
      let associatedShipments =
        cnIdsWithShs.find(({ cnId }) => cnId == cn.uuid)?.associatedShipments ??
        [];
      let shCosts =
        cnIdsWithShs.find(({ cnId }) => cnId == cn.uuid)?.shCost ?? [];
      return {
        uuid: cn.uuid,
...
async function main(From, Till, to, cc, plantNames = []) {
  ...
  try {
    if (monthDiff(new Date(From), new Date(Till)) > 3) {
      throw new Error(
        `Maximum allowed months difference is 3 found more than 3`
      );
    }
    const cnIdsWithSh = await getAllConsignments(From, Till, token, plantNames);

    const consignments = await getCnsMastersFromUUIDs(
      cnIdsWithSh.map((cnIdWithSh) => {
        return {
    ...
    const content = `Dear all, \n\nPlease find attached freight provision report from ${moment(
      new Date(From)
    ).format("DD-MM-YYYY")} till ${moment(new Date(Till)).format(
      "DD-MM-YYYY"
...
async function getAllConsignments(fromTime, tillTime, token, plantNames = []) {
  ...
  const allFields = ["consignment.uuid", "shCosts", "associatedShipments"];
  try {
...
async function _getJsonAoaArrayFromData(cns) {
  ...
  for (let cn of cns) {
    ...
    let town = cn.consignee?.address
      ? JSON.parse(cn.consignee.address)?.city
      : null;
    ...
    let cnInfoObj = {
      ...
      invoiceDate: cn?.consignmentDate
        ? moment(cn.consignmentDate).format("DD-MM-YYYY")
        : "-",
      eWayNo: cn?.eWayBillNumber ?? "",
      eWayDate: cn?.eWayBillExpiryDate
        ? moment(cn.eWayBillExpiryDate).format("DD-MM-YYYY")
        : "-",
      eWayTime: cn?.eWayBillExpiryDate
        ? moment(cn.eWayBillExpiryDate).format("hh:mm -A")
        : "-",
      distance: null,
      transporterName:
        sh?.fleetInfo?.fleetOwner?.name ??
        sh?.fleetInfo?.fleetOwner?.name ??
        "-",
      transporterGrnNo:
        cn?.customFields.find(({ fieldKey }) => fieldKey == "LR Number")
          ?.value ?? "-",
      transporterGrnDate:
        cn?.customFields.find(({ fieldKey }) => fieldKey == "LR Date")?.value ??
        "-",
      ...
      netWeight:
        parseFloat(
          (
            (cn?.loadInfo?.standardMeasurement?.weight?.netQuantity ?? 0.0) *
            1000
          ).toFixed(2)
        ) ?? "-",
      bucketQty: cn.lineItems.reduce((pv, cv) => {
        if (bucketTypeMaterialCodes.includes(cv?.material?.externalId)) {
          pv +=
            cnQtyAndItemIdMap[cv.uuid]?.packageMeasurement?.netQuantity ?? 0.0;
        }
        return pv;
      }, 0.0),
      "Bucket/Tub Weight Calculated as per Policy (In KG)": cn.lineItems.reduce(
        (pv, cv) => {
          if (bucketTypeMaterialCodes.includes(cv?.material?.externalId)) {
            pv +=
              (cnQtyAndItemIdMap[cv.uuid]?.weight?.netQuantity ?? 0.0) * 1000;
          }
          return pv;
        },
        0.0
      ...
      freightPaymentStatus:
        (cn?.customFields ?? []).find(
          ({ fieldKey }) => fieldKey == "Invoice Status"
        )?.value ?? "UNPAID",
      clubbingStatus: "-",
      toPayFreigt: "-",
      freightAmount: "-",
    ...
    if (dispatch && town) {
      ...
      var freightPrices = [];
      const freightPriceFromMap = freightPriceMap.filter(
        (_) => _.origin === dispatch && _.destination === town
      );
      ...
      distance =
        freightPrices?.[0]?.chargeTypes.find((e) => e.uuid == DISTANCE_UUID)
          ?.amount ?? 0;
    } else {
    ...
    if (distance == 0) {
      console.log(
        `Distance not found from freight pricing [${cnInfoObj.dispatch} to ${cnInfoObj.town}] calculting by lat-lng`
      );
    }
    cnInfoObj.distance =
      distance === 0
        ? calculateDistance(origin.center, destination.center)
        : distance;

    let freightCosts = cn?.shCosts;
    console.log(`freightCosts ${freightCosts}`)
    if (sh) {
      let freightAmount = (freightCosts ?? []).reduce((pv, cv) => {
        let item = (cv?.lineItems ?? []).find(
          (it) => it.consignmentId == cn.uuid
        );
        return pv + (item?.amount ?? 0.0);
      }, 0);

      console.log(`cnInfoObj.freightPaymentStatus ${cnInfoObj.freightPaymentStatus}`)
      cnInfoObj.toPayFreigt =
        cnInfoObj.freightPaymentStatus == "PAID"
          ? parseFloat(freightAmount.toFixed(2))
          : "-";
      cnInfoObj.freightAmount =
        cnInfoObj.freightPaymentStatus == "PAID"
          ? "-"
          : parseFloat(freightAmount.toFixed(2));
      cnInfoObj.clubbingStatus = (sh.shipmentStages ?? []).filter(
        ({ consignmentPickUps, consignmentDelivered }) =>
          !(
            (consignmentPickUps ?? []).includes(cn.uuid) ||
            (consignmentDelivered ?? []).includes(cn.uuid)
          )
      )?.length
        ? "Yes"
        : "No";
...
function aoa2XlsxFile(arrOfArr, fromDate, tillDate, fileName) {
  ...
  let headersArr = [
    ...
    [
      ...
      __styleCell(
        "Bucket/Tub Weight Calculated as per Policy (In KG)",
        true,
        14,
        border,
        "center",
      ...
      __styleCell(
        "Freight Payment Status (Paid/Unpaid)",
        true,
        14,
        border,
        "center",
...
main(1684132148000, 1686810548000, ["pooja.bishu@fretron.com"], [""])
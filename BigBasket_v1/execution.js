...
async function _getJsonAoaArrayFromData(cns) {
  ...
  for (let cn of cns) {
    ...
    if (dispatch && town) {
      ...
      const freightPrices = await getFreightPricing(dispatch, town)
    ...
    if (sh) {
      let freightCosts = await getShipmentCostByShId(sh.uuid)
...
main(1683443307000, 1686121707000, ["pooja.bishu@fretron.com"], ["pooja.bishu@fretron.com"])

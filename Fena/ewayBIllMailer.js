...
async function calculateDistanceByCordinates(origin, destination) {
    let url = `${FRT_PUB_BASE_URL}/itinerary/admin/calculateDistance?originLat=${origin[1]}&originLng=${origin[0]}&destinationLat=${destination[1]}&destinationLng=${destination[0]}`
    console.log(`URL : ${url}`)
    let res = await rp({
        method: "GET",
        uri: url,
...
async function main() {
    try {
        let isMailSend = true
        let origin = _.first($event.shipmentStages)?.place?.centerCoordinates ?? _.first($event.shipmentStages)?.hub?.centerCoordinates
        let destination = _.last($event.shipmentStages)?.place?.centerCoordinates ?? _.last($event.shipmentStages)?.hub?.centerCoordinates
        if (!origin || !destination) {
            console.log(`center cordinates not found For ${$event.shipmentNumber}`)
        } else {
            let totalDistance = await calculateDistanceByCordinates(origin, destination)
            totalDistance = totalDistance ? parseFloat((totalDistance / 1000).toFixed(2)) : 0
            console.log(`Total Distance ${totalDistance} between origin ${origin} and lastDest ${destination}`)
            isMailSend = (totalDistance > 200)
        }
            ...
            if (ewayBillAlert && true) {
                if (ewayBillAlert?.description?.includes(invoice)) {
                    ...
                    if (isMailSend) {
                        ...
                        console.log(`Not Sending Mail Total Distance Under 200Km`)
                    }

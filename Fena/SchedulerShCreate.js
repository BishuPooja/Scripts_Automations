...
async function getOrderById(orderId) {
    ...
    try {
        ...
        if (res.status == 200) {
            ...
            console.log(`Get orderById ${orderId} response error : ${res.error}`)
        }
    } catch (e) {
...
async function addCfsOnShCreate(shId, cfPayload) {
    try {
...
async function getShsByFilter(filter) {
    let url = `${FRT_PUB_BASE_URL}/shipment-view/shipments/v1?filters=${encodeURIComponent(JSON.stringify(filter))}`;
    console.log(url)
    try {
        let res = await rp({
            uri: url,
        ...
        console.log(`Error in getting Shs by filter ${e.message}`)
    }
    return []
}

async function getOrdersByFUID(freightUnitLineItemId) {
    try {
        let orderNos = []
        ...
        console.log(`error in getting orders By fuId ${freightUnitLineItemId} ${e.message}`)
    }
}

async function getSapPayload(sh) {
    try {
        let shId = sh.uuid
        let orderNos = []
        let freightUnitLineItemId = sh.freightUnitLineItemId
        if (freightUnitLineItemId) {
            orderNos = await getOrdersByFUID(freightUnitLineItemId)
        }
        ...
        console.log(`error in getting Sap Payload ${e.mesage}`)
    }
}
    ...
    try {
        let filter = {
            ...
            "_not": {
                ...
                "_shcf_Is Shipment Data Sent": ["Yes"]
            }
        }
        let shipments = await getShsByFilter(filter)
        console.log(`Total Shs of Data Sent Failed ${shipments.length}`)
        for (let sh of shipments) {
            try {
                let shId = sh.uuid
                let sapPayload = await getSapPayload(sh)
                ...
                await addCfsOnShCreate(shId, cfPayload)
            } catch (e) {
                console.log(`Error Executing Shipment ${sh?.shipmentNumber} ${e.message}`)
            }
        }
    } catch (e) {
        console.log(`Catched error : ${e.message}`)
...
try {
    ...
    console.log(`error in Scheduler ${e.message}`)
}
